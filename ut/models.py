...
class Attributes(models.Model):
    ...
    def FT_Contains(self):
        return 'lower({})'.format(self.ValueField) + " like lower('%{val}%')"
...
class Filters(models.Model):
    ...
    filtertypes = [(FT_Exact, 'Exact'), (FT_MinMax, 'Min and Max'), (FT_Contains, 'Contains'),
                   (FT_Like, 'SQL Style LIKE')]
...
class Instances(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if user.is_superuser or Classes.objects.filter(id=Class_id,
            ...
            if self.pk is None:
                raise Exception(
                    "User {} does not have rights to insert record of class {}".format(user.username, self.Class.Class))
            else:
                raise Exception(
                    "User {} does not have rights to update record of class {}".format(user.username, self.Class.Class))
...
class Values(models.Model):
    ...
    def save(self, *args, **kwargs):  # rewrite save to check for unique & non null values
        att = self.Attribute
        # Add Datatypes Here
        Qs = {DT_Integer: self.int_value, DT_Float: self.float_value, DT_String: self.char_value,
              DT_Text: self.text_value,
              DT_Time: self.char_value, DT_Date: self.datetime_value, DT_Datetime: self.datetime_value,
              DT_Instance: self.instance_value, DT_Email: self.char_value, DT_Hyperlink: self.char_value}
        Qn = {DT_Integer: 'int_value', DT_Float: 'float_value', DT_String: 'char_value', DT_Text: 'text_value',
              DT_Time: 'char_value', DT_Date: 'datetime_value', DT_Datetime: 'datetime_value',
              DT_Instance: 'instance_value',
              DT_Email: 'char_value', DT_Hyperlink: 'char_value'}
        val = 0
        # handle unique
        if att.UniqueAtt:
            ...
            if val.count() > 0:
                raise Exception(
                    'Attribute "{}" for class "{}" is unique and the value "{}" has already used in the instance with id {}.'.format(
                        att.Attribute, att.Class.Class, Qs[att.DataType.id], val[0].Instance.id))
